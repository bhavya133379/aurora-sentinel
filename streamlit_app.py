...
def get_priority_ml(crime_type, description):
    """
    Predicts priority. 
    INCLUDES SAFETY RULE: Kidnapping/Emergency/Murder are ALWAYS High.
    """
    # 1. SAFETY RULES (Override ML for critical crimes)
    critical_crimes = ["Kidnapping", "Murder", "Emergency", "Robbery", "Domestic Violence"]
    if crime_type in critical_crimes:
        return "High"
    
    # 2. ML Prediction for others
    try:
        if not description or description.strip() == "": desc = "general incident"
        else: desc = description
        input_data = pd.DataFrame({'crime_type': [crime_type], 'description': [desc]})
...
# ================================================================
#  TRANSLATIONS (FULL RESTORED & UPDATED)
# ================================================================
...
ALL_T = {
    "English": {
        ...
        "legend_medium": "Medium Priority",
        "legend_low": "Low Priority",
        # Added for HTML Component translations
        "html_loc_detected": "Location Detected!",
        "html_lat": "Lat",
        "html_lon": "Lon",
        "html_warn_enter": "⚠️ IMPORTANT: Click boxes below & press ENTER.",
    ...
    "Hindi": {
        ...
        "html_lat": "अक्षांश",
        "html_lon": "देशांतर",
        "html_warn_enter": "⚠️ महत्वपूर्ण: नीचे बॉक्स पर क्लिक करें और ENTER दबाएं।",
        "html_err_denied": "❌ स्थान अनुमति अस्वीकार।",
        "html_detecting": "📡 पता लगा रहा है…",
        "html_btn_detected": "✅ GPS पता चला",
        "fetching_addr": "📍 GPS पता प्राप्त कर रहे हैं..."
...
def get_place_name(lat, lon):
    try:
        r = requests.get("https://nominatim.openstreetmap.org/reverse", params={"lat": lat, "lon": lon, "format": "json"}, headers={"User-Agent": "AuroraSentinel/1.0"}, timeout=8)
...
def auto_gps_component(lat_label: str, lon_label: str, btn_label: str, t_dict):
    ...
    <style>
      ...
      #gps-btn:disabled{{background:#888;cursor:not-allowed;}}
      #coord-box{{display:none;padding:10px 14px;background:#0f2e1e;color:#00ff88;font-family:monospace;border-radius:8px;border:1px solid #00ff88;line-height:1.6;font-size:14px;}}
      .warn-txt{{color:#ffcc00; font-weight:bold; font-size:13px; margin-top:8px;}}
      #err-box{{display:none;color:#ff4444;font-size:13px;margin-top:8px;}}
    </style>
    <button id="gps-btn" onclick="detectGPS()">{btn_label}</button>
    <div id="coord-box">✅ <b>{t_dict['html_loc_detected']}</b><br>📍 {t_dict['html_lat']}: <span id="lat-disp">—</span> &nbsp;|&nbsp; {t_dict['html_lon']}: <span id="lon-disp">—</span>
...
def render_dashboard_charts(df_reports, df_sos, t):
    # Calculate Counts
    high_count = len(df_reports[(df_reports["priority"] == "High") & (df_reports["crime_type"] != "Emergency")])
    emergency_count = len(df_reports[df_reports["crime_type"] == "Emergency"])
    sos_count = len(df_sos)
    medium_count = len(df_reports[df_reports["priority"] == "Medium"])
    low_count = len(df_reports[df_reports["priority"] == "Low"])
    ...
    # Only keep non-zero values to avoid clutter
    filtered_labels = []
    filtered_sizes = []
    for l, s in zip(labels, sizes):
        if s > 0:
    ...
    # Filter colors to match filtered data
    # (This is a bit tricky because we need to maintain color mapping, 
    # but for simplicity in this logic, we just slice the standard colors list 
    # or map specifically. Let's map specifically to maintain consistent color coding.)
    
    color_map = {
        t["legend_high_crimes"]: "#e74c3c", # Red
        t["legend_emergencies"]: "#c0392b", # Dark Red
        t["legend_sos"]: "#e67e22",         # Orange
        t["legend_medium"]: "#f1c40f",      # Yellow
        t["legend_low"]: "#2ecc71"          # Green
    }
    ...
    wedges, texts, autotexts = ax.pie(
        ...
        colors=final_colors, 
        autopct=lambda p: '{:.0f}'.format(p * sum(filtered_sizes) / 100) if p > 0 else '',
        startangle=90, 
        pctdistance=0.85,
...
if menu == t["navigation"][0]:
    ...
    if emergency_image is not None:
        ...
        if st.session_state.last_emg_file != current_file_sig:
            ...
            assigned_station = AREA_TO_STATION.get(location_en, location_en)
            place_name = ""; lat_val = None; lon_val = None
            if emg_lat and emg_lon:
                try: lat_val = float(emg_lat); lon_val = float(emg_lon); place_name = get_place_name(lat_val, lon_val)
                except: pass
            ...
            cursor.execute("INSERT INTO reports (case_id, crime_type, location, latitude, longitude, place_name, description, evidence, priority, status, timestamp, assigned_station) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                (case_id, "Emergency", location_en, lat_val, lon_val, place_name, "One-Tap Emergency Image Submission", img_filename, predicted_priority, "CRITICAL", timestamp, assigned_station))
            conn.commit()
            
            st.success(f"{t['emg_submitted']} Case ID: {case_id}")
            st.error(f"{t['routed_to']} {assigned_station} Station")
            st.markdown("---")
        else:
            st.info("✅ This emergency report has already been submitted.")
    ...
    if st.button(t["submit"], type="primary"):
        ...
        lat_val = lon_val = None; place_name = ""
        if lat and lon:
            try: lat_val = float(lat); lon_val = float(lon); place_name = get_place_name(lat_val, lon_val)
            except ValueError: st.error(t["location_error"]); st.stop()
        ...
        st.info(f"📌 **{t['case_id_label']} {case_id}**")
        st.info(f"🔥 **Predicted Priority: {priority}**")
...
# ================================================================
    ...
    st.subheader(t["track_title"])
    cursor.execute("SELECT case_id,crime_type,location,timestamp FROM reports ORDER BY id DESC")
    cases = cursor.fetchall()
    if not cases: st.info(t["no_cases"])
    else:
        case_options = {f"{c['case_id']} | {c['crime_type']} | {c['location']} | {c['timestamp']}": c["case_id"] for c in cases}
        selected_case = st.selectbox(t["select_case"], options=list(case_options.keys()))
        if st.button(t["track_btn"]):
            cid = case_options[selected_case]
            cursor.execute("SELECT * FROM reports WHERE case_id = ?", (cid,))
            case = cursor.fetchone()
            if case:
                st.success(t["case_found"])
                col1, col2 = st.columns(2)
                ...
                with col2: st.write(t["status_label"], case["status"]); st.write(t["priority_label"], case["priority"])
                if case["latitude"] is not None:
                    st.write(t["lat_label"], f"{case['latitude']:.6f}"); st.write(t["lon_label"], f"{case['longitude']:.6f}")
                    if case["place_name"] and case["place_name"].strip(): st.write(t["gps_place"], case["place_name"])
                    else: st.write(t["gps_place"], get_place_name(case["latitude"], case["longitude"]))
                else: st.info(t["no_gps"])
    ...
    if not st.session_state.police_logged:
        ...
        with tab1:
            df_reports = pd.read_sql_query(
                "SELECT * FROM reports WHERE assigned_station=? OR assigned_station IS NULL OR assigned_station='' OR crime_type='Emergency' ORDER BY id DESC", 
            ...
            if not df_reports.empty or not df_sos.empty:
                c1, c2, c3 = st.columns(3)
                total_count = len(df_reports) + len(df_sos)
                high_count = len(df_reports[df_reports["priority"]=="High"])
                closed_count = len(df_reports[df_reports["status"]=="Closed"])
                
                c1.metric(t["total_cases"], total_count)
                c2.metric(t["urgent_cases"], high_count + len(df_sos)) # SOS are urgent too
                c3.metric(t["closed_cases"], closed_count)
        ...
        with tab2:
            ...
            if not reports: st.info(t["no_reports"])
                ...
                for r in reports:
                    with st.expander(f"📌 {r['case_id']} | {r['crime_type']} | {r['priority']}"):
                        col1, col2 = st.columns(2)
                        with col1: st.write(t["case_id_label"], r["case_id"]); st.write(t["crime_type"]+":", r["crime_type"]); st.write(t["location_label"], r["location"]); st.write(t["time_label"], r["timestamp"])
                        with col2: st.write(t["priority_label"], r["priority"]); st.write(t["status_label"], r["status"])
                        
                        if r["latitude"] is not None and r["longitude"] is not None:
                            st.write(t["gps_coords"], f"{r['latitude']:.6f}, {r['longitude']:.6f}")
                            
                            if r["place_name"] and r["place_name"].strip():
                                st.write(t["gps_loc_label"], r["place_name"])
                            else:
                                with st.spinner(t["fetching_addr"]):
                                    addr = get_place_name(r["latitude"], r["longitude"])
                                st.write(t["gps_loc_label"], addr)
                                cursor.execute("UPDATE reports SET place_name=? WHERE id=?", (addr, r["id"]))
                                conn.commit()
                        else:
                            st.info(t["no_gps"])
                        
                        st.info(r["description"] or t["no_description"])
                        if r["evidence"] and r["evidence"] != "No Evidence":
                            if r["evidence"].startswith("Images:"):
                                for img in r["evidence"].replace("Images:","").split("||"):
                                    ip = img.strip()
                                    if not os.path.isabs(ip): ip = os.path.join(IMAGE_FOLDER, ip)
                                    if os.path.exists(ip): st.image(ip, width=250)
                            elif r["evidence"].endswith((".jpg", ".png", ".jpeg")):
                                ip = os.path.join(IMAGE_FOLDER, r["evidence"])
                                if os.path.exists(ip): st.image(ip, width=250)
                            elif r["evidence"].startswith("PDF:"): st.info("📄 " + r["evidence"].replace("PDF:",""))
                        new_status = st.selectbox(t["update_status"], t["status_options"], key=f"status_{r['id']}")
                        if st.button(t["save"], key=f"save_{r['id']}"):
                            en_status = ALL_T["English"]["status_options"][t["status_options"].index(new_status)]
                            cursor.execute("UPDATE reports SET status=? WHERE id=?", (en_status, r["id"])); conn.commit(); st.success(t["status_updated"]); st.rerun()
        
        with tab3:
            st.subheader(t["sos_tab"])
            alerts = cursor.execute("SELECT * FROM sos_alerts ORDER BY id DESC").fetchall()
            if not alerts: st.info(t["no_sos"])
            else:
                for s in alerts:
                    with st.expander(f"🚨 SOS #{s['id']} | {s['timestamp']}"):
                        st.write(f"{t['status_label']} {s['status']}")
                        ...
                        if s["latitude"] is not None:
                            st.write(f"{t['gps_coords']} {s['latitude']:.6f}, {s['longitude']:.6f}")
                            
                            if s["place_name"] and s["place_name"].strip():
                                st.write(f"{t['gps_loc_label']} {s['place_name']}")
                            else:
                                with st.spinner(t["fetching_addr"]):
                                    addr = get_place_name(s["latitude"], s["longitude"])
                                st.write(f"{t['gps_loc_label']} {addr}")
                                cursor.execute("UPDATE sos_alerts SET place_name=? WHERE id=?", (addr, s["id"]))
                                conn.commit()
                        else:
                            st.info(t["no_gps"])
...
# ================================================================
    ...
    st.subheader(t["sos_title"]); st.warning(t["sos_gps_info"])
    auto_gps_component(t["sos_lat_input"], t["sos_lon_input"], t["gps_btn_sos"], t)
    col1, col2 = st.columns(2)
    with col1: sos_lat = st.text_input(t["sos_lat_input"], key="sos_lat", placeholder=t["sos_lat_ph"])
    with col2: sos_lon = st.text_input(t["sos_lon_input"], key="sos_lon", placeholder=t["sos_lon_ph"])
    if st.button(t["send_sos"], type="primary"):
        if not sos_lat or not sos_lon: st.error(t["sos_no_coords"])
        else:
            try:
                lat_val = float(sos_lat); lon_val = float(sos_lon)
                place = get_place_name(lat_val, lon_val)
                cursor.execute("INSERT INTO sos_alerts (location,latitude,longitude,place_name,status,timestamp) VALUES (?,?,?,?,?,?)", (place, lat_val, lon_val, place, "CRITICAL", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                conn.commit(); st.success(t["sos_sent"]); st.info(f"{t['sos_your_loc']} {place}")
            except: st.error(t["location_error"])

>>>>>>> 853d2caf6964cd8efa5d43087fd25a2498fdd2df
conn.close()